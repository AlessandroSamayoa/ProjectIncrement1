...
namespace GameProject
{
    ...
    public class Game1 : Game
    {
        ...
        private void SpawnBear()
        {
            ...
            float degree = RandomNumberGenerator.NextFloat((float)Math.PI * 2);
            float velocity = GameConstants.MinBearSpeed + RandomNumberGenerator.NextFloat(GameConstants.BearSpeedRange);
            Vector2 bearVector = new Vector2(velocity * (float)Math.Cos(degree), velocity * (float)Math.Sin(degree));

            // create new bear
            TeddyBear teddy = new TeddyBear(Content, @"graphics\teddybear", x, y, bearVector, null, null);
